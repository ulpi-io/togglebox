...
export const ExperimentVariationSchema = z.object({
  key: z.string().min(1, 'Variation key is required').regex(
    /^[a-z][a-z0-9_]*$/,
...
const ExperimentBaseSchema = z.object({
  ...
  experimentKey: z.string().min(1, 'Experiment key is required').regex(
    /^[a-z][a-z0-9_-]*$/,
  ...
  variations: z.array(ExperimentVariationSchema).min(2, 'At least 2 variations required'),
