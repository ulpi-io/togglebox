...
export default function ExperimentDetailPage({
  ...
  const loadExperiment = useCallback(async () => {
    try {
      ...
      if (
        ...
        try {
          const resultsData = await getExperimentResultsApi(
            platform,
            environment,
            experimentKey,
          );
  ...
  const saveTrafficAllocation = async () => {
    ...
    try {
      const updated = await updateExperimentTrafficApi(
        platform,
        environment,
        experimentKey,
        editedTraffic,
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          ...
          {experiment.status === "draft" && (
            <Link
              href={`/platforms/${platform}/environments/${environment}/experiments/${experimentKey}/edit`}
            >
            ...
            {isEditingTraffic ? (
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <Button
                    variant="outline"
                    onClick={cancelEditingTraffic}
                    disabled={isSavingTraffic}
                  >
                    Cancel
                  </Button>
                  <Button
                    onClick={saveTrafficAllocation}
            ...
            <div className="border border-info/50 bg-info/10 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  {experiment.primaryMetric.eventName}
                </div>
                <div>
                  <span className="font-bold">Type:</span>{" "}
                  {experiment.primaryMetric.metricType}
                </div>
                <div>
                  <span className="font-bold">Direction:</span>{" "}
                  {experiment.primaryMetric.successDirection}
                </div>
              </div>
            </div>
