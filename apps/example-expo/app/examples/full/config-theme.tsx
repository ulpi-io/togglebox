...
export default function ConfigThemeScreen() {
  ...
  const theme = useMemo(() => ({ ...defaultTheme, ...themeConfig }), [themeConfig])
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonRow}>
        <TouchableOpacity style={[styles.button, { backgroundColor: theme.primaryColor as string, borderRadius }]}>
          <Text style={styles.buttonText}>Primary</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.button, { backgroundColor: theme.accentColor as string, borderRadius }]}>
...
function ThemeRow({ label, value, isColor }: { label: string; value: string; isColor?: boolean }) {
  return (
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 13, fontWeight: '600', marginBottom: 12, color: '#374151' },
  themeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  ...
  colorSwatch: { width: 20, height: 20, borderRadius: 4, borderWidth: 1, borderColor: '#e5e7eb' },
  refreshButton: { backgroundColor: '#3b82f6', padding: 14, borderRadius: 8, alignItems: 'center' },
