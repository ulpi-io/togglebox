...
export default function Page() {
  const { experiment, getVariant, isLoading } = useExperiment('cta-experiment', { userId: 'user-123' })
  ...
  const variants: Record<string, { text: string; bg: string; description: string }> = {
    control: { text: 'Get Started', bg: 'bg-gray-600 hover:bg-gray-700', description: 'Control: Neutral gray' },
    'variant-a': { text: 'Start Free Trial', bg: 'bg-blue-600 hover:bg-blue-700', description: 'Variant A: Free trial messaging' },
    'variant-b': { text: 'Try It Now!', bg: 'bg-green-600 hover:bg-green-700', description: 'Variant B: Urgency messaging' },
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8">
      ...
      <button onClick={handleClick} className={`px-8 py-4 text-white text-xl font-medium rounded-lg ${config.bg}`}>
        {config.text}
      </button>
