...
export default function Page() {
  ...
  return (
    <div className={`min-h-screen p-8 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white'}`}>
      <h1 className="text-2xl font-bold mb-6">Feature Flags</h1>

      <div className="max-w-md space-y-4">
        <div className={`p-4 rounded-lg ${darkMode ? 'bg-green-900' : 'bg-green-50'} border border-green-500`}>
          <p className="text-sm mb-1">dark-mode flag</p>
          <p className="text-lg font-semibold">{darkMode ? 'ENABLED' : 'DISABLED'}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {flags.map((f) => (
            <div key={f.flagKey} className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${f.enabled ? 'bg-green-500' : 'bg-gray-400'}`} />
