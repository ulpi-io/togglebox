...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Check if reviews feature is enabled for this user
    const showReviews = await togglebox.isFlagEnabled('reviews-enabled', context)

    // Check if express shipping is available
    const expressShipping = await togglebox.isFlagEnabled(
      'express-shipping',
      context
    )
